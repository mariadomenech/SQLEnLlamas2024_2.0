...
BEGIN
	...
	BEGIN
		...
		SELECT CONCAT (
				...
				,' es de '
				,(
					SELECT depositos - compras - retiros
					FROM TRANSACCIONES_CLIENTE
					)
				,' Euros'
				) AS MENSAJE
	END
	ELSE IF @Calculo = 2 -- TOTAL DEPOSITOS
	BEGIN
		SELECT CONCAT (
	...
	END
	ELSE IF @Calculo = 3 -- TOTAL DE COMPRAS
	BEGIN
		SELECT CONCAT (
	...
	END
	ELSE IF @Calculo = 4 -- TOTAL DE RETIROS
	BEGIN
		SELECT CONCAT (
...
EXEC SDF_CASO3_RETO4 28, '202004', 1;
EXEC SDF_CASO3_RETO4 1, '202001', 2;
