...
set @total = 
 case 
	 when (@operacion = 1) then
		(select [CIVICA\regina.margolles].[RMC_Caso3_Ejercicio4_Balance] (@customer_id, @mes))
	 when (@operacion = 2) then
	    (select [CIVICA\regina.margolles].[RMC_Caso3_Ejercicio4_Deposit] (@customer_id, @mes))
	 when (@operacion = 3) then		
		(select  [CIVICA\regina.margolles].[RMC_Caso3_Ejercicio4_Purchase] (@customer_id, @mes))	  
	 when (@operacion = 4) then
		(select [CIVICA\regina.margolles].RMC_Caso3_Ejercicio4_Withdrawal (@customer_id,@mes))
	 end
