...
(Select c.order_id,
		...
		CASE
	    	...
	    	END as price_pizza,
		(cast(r.distance as decimal(5,2)) * 0.30) as travel_cost,
		 (SELECT count(*) FROM STRING_SPLIT(c.extras,',')) as extras_pizza
 from CTE_customer_orders_clean as c
     left join CTE_runner_orders_clean as r
	on c.order_id = r.order_id
     inner join case02.pizza_names as p
...
select CTE1.order_id,
	   CTE1.travel_cost,
       SUM(CTE1.price_pizza + (CTE1.extras_pizza)) over (partition by CTE1.order_id) as subtotal
from CTE1
),
...
select  distinct (CTE2.order_id),
		(CTE2.subtotal - CTE2.travel_cost) as profit

from CTE2

)
select SUM(profit) as profit from CTE3;
