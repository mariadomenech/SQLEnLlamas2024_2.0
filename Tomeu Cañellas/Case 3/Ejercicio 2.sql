...
Está perfecto. Sencillo y legible.
Como curiosidad, para que los meses en vez de salir en número te ponga el mismo formato que la solución de la app puedes
usar FORMAT(txn_date, 'MMM', 'ES'):

WITH txn_per_month AS (
    SELECT  customer_id, 
	    FORMAT(txn_date, 'MMM', 'ES') AS month,
            YEAR(txn_date) AS year,
            txn_type,
            COUNT(*) AS txn_count
    FROM [SQL_EN_LLAMAS_ALUMNOS].[case03].[customer_transactions]
